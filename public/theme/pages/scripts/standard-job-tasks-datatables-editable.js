    ...
    var handleTable = function () {
        ...
        function saveRow(oTable, nRow) {
            ...
            ajax = $.ajax({
                ...
                type: 'POST',
                url: '/edit-lookups/standard-job-tasks/save',
                dataType: 'json',
                encode: true,
                data: {
                    index: index,
                ...
                .done(function(response) {
                    ...
                    if (response == 'OK') {
                        ...
                        $('#error_ajax').find('p').html('This Standard Job Tasks is already used! Be careful');
        ...
        function loadTasks(oTable) {
            standard_job = $('.select2me').val();
            var el = $('.portlet-body');
            ...
            ajax = $.ajax({
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                type: 'POST',
                url: '/edit-lookups/standard-job-tasks/upload',
                dataType: 'json',
                encode: true,
                data: {
                    standard_job_id: standard_job
                ...
                .done(function(response) {
                    ...
                    $.each(response, function(index, line){
                        var aiNew = oTable.fnAddData([line.name, '<a class="edit" href="javascript:;"> Edit </a>', '<a class="delete" href="javascript:;" data-toggle="confirmation" data-singleton="true"> Delete </a>']);
                ...
                .error(function() {
                    App.unblockUI(el);
                    $('#error_ajax').find('p').html('Cannot load Standard Job Tasks!<br>Some errors occurred while getting data from the server!');
                    $('#error_ajax').modal('show');
                });

        }

        var table = $('#standard_job_tasks');
        var standard_job;
        ...
        var tableWrapper = $("#standard_job_tasks_wrapper");
        ...
        $('.select2me').on('change', function () {
            if($(this).val() != null) loadTasks(oTable);
        });

        $('#standard_job_tasks_new').click(function (e) {
            e.preventDefault();
        ...
        table.on('click', '.delete', function (e) {
            ...
            ajax = $.ajax({
                headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                type: 'POST',
                url: '/edit-lookups/standard-job-tasks/delete',
                dataType: 'json',
                encode: true,
                data: {
                    index: index,
